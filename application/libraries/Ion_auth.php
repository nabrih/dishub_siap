...
class Ion_auth
{
	...
	/**
	* cek for user access role
	* @author nabrih
	* @return boolean
	*/
	public function check_access($menu_name)
	{
		$this->load->model('acl_model');

		if ($this->logged_in()){
			$result = $this->acl_model->checkMenu($menu_name);
			if ($result) {
				return true;
			}else{
				echo "<div>You don't have permission to access </div>";
				die();
			}
		}else{
	...
	*/
	public function allowed_menu()
	{
		...
		$result = $this->acl_model->get_allowed_menu();
		
		if ($result) {
			foreach ($result->result_array() as $row)
			{
			    $this->data['acl'][$row['menu_name']]=1;
			}

			//implement in the view sidebar menu or others
			/*
			if(array_key_exists('nama_menu', $acl)){
				to do 
			}
			*/
			// echo array_key_exists('banner', $this->$data['acl']);
		}

		return $this->data['acl'];

		// echo json_encode($this->data['acl']);
	}
